...
if [[ "$MODE" == "--publish" ]]; then
    # Fail before publishing if the required mirror cannot be reached.
    ssh -i "${SCREENOFF_MIRROR_SSH_KEY:-$HOME/Projects/keys/id}" \
        -o IdentitiesOnly=yes -o IdentityAgent=none -o BatchMode=yes \
        -o StrictHostKeyChecking=yes -o ConnectTimeout=15 \
        "${SCREENOFF_MIRROR_HOST:-root@5.78.223.34}" \
        'test -d /var/www/screenoff-updates && test -w /var/www/screenoff-updates'
    ...
    "$ROOT_DIR/Script/sync_update_mirror.sh" "$TAG" "$RELEASE_DIR"
fi
