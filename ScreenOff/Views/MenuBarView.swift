...
/// 菜单栏快捷窗口。304 pt 紧凑布局：主操作 → 亮度 → 屏幕关闭 → 保持唤醒 → 页脚。
struct MenuBarView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            autoOffSection
            keepAwakeSection.padding(.top, 12)
            if let status {
                statusFooter(status)
            }
            Divider().padding(.vertical, 12)
    ...
    private var primaryAction: some View {
        Button {
            ...
            HStack(spacing: 7) {
                (controller.screenState == .off ? MenuBarGlyph.outline : MenuBarGlyph.filled).image
                    .resizable()
                    .aspectRatio(contentMode: .fit)
                    .frame(width: 15)
                Text(controller.screenState == .off ? "点亮屏幕" : "关闭屏幕")
    ...
    private var primaryActionHint: String? {
        controller.screenState == .off ? "按本机任意键" : preferences.screenOffShortcut?.description
    }

    private var primaryActionHelp: String {
        guard controller.canControlDisplay else { return "当前设备无法调节内建屏幕亮度" }
        if controller.screenState == .off {
            return "按本机任意键或使用触控板可点亮；远程操作时，可点击此按钮点亮屏幕。"
        }
        return "关闭内建屏幕背光；快捷键只执行关闭，不会切换为点亮。"
    }
        ...
        HStack {
            ...
            Button {
                ...
                Label("退出", systemImage: "power")
                    .font(.system(size: 13, weight: .medium))
                    .frame(minWidth: 60, minHeight: 30, alignment: .trailing)
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
            .foregroundStyle(.secondary)
